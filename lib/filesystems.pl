...
swish_config:source_alias(filesystem).

:- http_handler(swish(list_filesystems), list_filesystems, [id(swish_filesystems)]).


%%	list_filesystems(+Request)
%
%	Get a list of registered filesystem code. Examples are described in
%	a file swish_filesystems('index.json').

list_filesystems(_Request) :-
	http_absolute_location(swish(filesystem), HREF, []),
	findall(Index,
		absolute_file_name(filesystem(.), Index,
				   [ access(read),
				     file_type(directory),
				     file_errors(fail),
				     solutions(all)
